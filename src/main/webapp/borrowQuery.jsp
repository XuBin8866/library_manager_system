<%@page import="com.xxbb.actionform.BookBorrowForm"%>
<%@ page contentType="text/html; charset=utf-8" language="java" import="java.sql.*" errorPage="" %>
<%@ page import="java.util.*"%>
<html>
<head>
<title>图书馆管理系统</title>
<link href="CSS/style.css" rel="stylesheet">
<script src="JS/function.js"></script>
<script language="javascript">
function check(myform){
	if(myform.flag[0].checked&&myform.flag[1].checked){
		alert("请只选择一种查询方式！");
		return false;
	}
	if(myform.flag[0].checked==false && myform.flag[1].checked==false){
		alert("请选择查询方式!");return false;
	}
	if (myform.flag[1].checked){
		if(myform.sdate.value==""){
			alert("请输入开始日期");myform.sdate.focus();return false;
		}		
		if(CheckDate(myform.sdate.value)){
			alert("您输入的开始日期不正确（如：2011-02-14）\n 请注意闰年!");myform.sDate.focus();return false;
		}
		if(myform.edate.value==""){
			alert("请输入结束日期");myform.edate.focus();return false;
		}		
		if(CheckDate(myform.edate.value)){
			alert("您输入的结束日期不正确（如：2011-02-14）\n 请注意闰年!");myform.edate.focus();return false;
		}
	}
}
</script>
</head>
<body onLoad="clockon(bgclock)">
<%@include file="banner.jsp"%>
<%@include file="navigation.jsp"%>
<table width="778"  border="0" cellspacing="0" cellpadding="0" align="center">
  <tr>
    <td valign="top" bgcolor="#FFFFFF"><table width="99%" height="510"  border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" class="tableBorder_gray">
  <tr>
    <td height="510" valign="top" style="padding:5px;"><table width="98%" height="487"  border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td height="22" valign="top" class="word_orange">当前位置：系统查询 &gt; 图书借阅查询 &gt;&gt;&gt;</td>
      </tr>
      <tr>
        <td align="center" valign="top">
		<form name="myform" method="post" action="borrow?action=borrowQuery">
          <table width="98%" height="67"  border="0" cellpadding="0" cellspacing="0" bgcolor="#E3F4F7" class="tableBorder_gray">
            <tr>
              <td rowspan="2" align="center" bgcolor="#F9D16B">&nbsp;<img src="Images/search.gif" width="45" height="28"></td>
              <td height="29" bgcolor="#F9D16B"><input name="flag" type="checkbox" class="noborder" value="item" <c:if test="${supfunction=='item' }">checked="checked"</c:if>>
                请选择查询依据：
                <select name="f" class="wenbenkuang" id="f">
                  <option value="barcode" <c:if test="${function=='barcode' }">selected="selected"</c:if> >
                  图书条形码</option>
                  <option value="bookname" <c:if test="${function=='bookname' }">selected="selected"</c:if>>
                  图书名称</option>
                  <option value="readerbarcode" <c:if test="${function=='readerbarcode' }">selected="selected"</c:if>>
                  读者条形码</option>
                  <option value="readername" <c:if test="${function=='readername' }">selected="selected"</c:if>>
                  读者名称</option>
                  </select>
                  <input name="key" type="text" id="key" size="50" value="${value }"   />
                  <input name="Submit" type="submit" class="btn_grey" value="查询" onClick="return check(myform)"></td>
            </tr>
            <tr>
              <td height="26" bgcolor="#F9D16B">
                <input name="flag" type="checkbox" class="noborder" value="date" <c:if test="${supfunction=='date' }">checked="checked"</c:if>>
                借阅时间：                从
                <input name="sdate" type="text" id="sdate" value="${startValue }">
                到
                <input name="edate" type="text" id="edate" value="${endValue }">
                (日期格式为：2011-02-14)</td>
            </tr>
          </table>		
		</form>
          <table width="100%" height="30"  border="0" cellpadding="0" cellspacing="0">
            <tr>
              <td height="36" align="center">暂无图书借阅信息！</td>
            </tr>
          </table>
          <table width="98%"  border="1" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" bordercolordark="#F6B83B" bordercolorlight="#FFFFFF">
  <tr align="center" bgcolor="#e3F4F7">
    <td width="11%" bgcolor="#F9D16B">图书条形码</td>
    <td width="29%" bgcolor="#F9D16B">图书名称</td>
    <td width="15%" bgcolor="#F9D16B">读者条形码</td>
    <td width="13%" bgcolor="#F9D16B">读者名称</td>
    <td width="12%" bgcolor="#F9D16B">借阅时间</td>
    <td width="12%" bgcolor="#F9D16B">应还时间</td>
    <td width="8%" bgcolor="#F9D16B">是否归还</td>
  </tr>
  <c:forEach items="${borrowquery }" var="bq">
  <tr> 
    <td style="padding:5px;">&nbsp;${bq.bookBarcode }</td>
    <td style="padding:5px;">${bq.bookName }</td>
    <td style="padding:5px;">&nbsp;${bq.readerBarcode }</td>
    <td style="padding:5px;">&nbsp;${bq.readerName }</td>
    <td style="padding:5px;">&nbsp;${bq.borrowTime }</td>
    <td style="padding:5px;">&nbsp;${bq.returnTime }</td>
    <td align="center" style="padding:5px;">&nbsp;
    <c:if test="${bq.ifBack==0 }">未归还</c:if>
    <c:if test="${bq.ifBack==1 }">已归还</c:if>
    </td>
  </tr>
  </c:forEach>

</table>
          </td>
      </tr>
    </table>
</td>
  </tr>
</table><%@ include file="copyright.jsp"%></td>
  </tr>
</table>
</body>
</html>
